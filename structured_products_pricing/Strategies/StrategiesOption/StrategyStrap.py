...
class StrategyStrap(StrategyBase):
    """
    Class to define a Strap strategy, extending from StrategyBase.

    A Strap involves:
    - Long two calls and one put at the same strike and maturity,
    """
    def __init__(self, MarketObject: Market, PricerObject: PricerBase, strike: float, maturity_date: datetime):
        ...
        - strike: float. Common strike price for both the calls and the put.
        - maturity_date: datetime. Common maturity date for all options.
        """
        super().__init__(MarketObject, PricerObject)
        self.strategy_name = "Strap"
        # Define the European call and put options at the same strike
