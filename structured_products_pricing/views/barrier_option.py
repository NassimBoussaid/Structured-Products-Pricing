...
def run():
    ...
    with col_right:
        ...
        if price_button:
            with st.spinner("Pricing in progress..."):
                ...
                option = OptionBarrier(
                    cp_type,
                    strike,
                    datetime.combine(maturity_date, datetime.min.time()),
                    knock_type,
                    direction,
                    barrier_level,
                    barrier_exercise
                ...
                # Setup Pricer
                pricer = PricerMC(
                    datetime.combine(pricing_date, datetime.min.time()),
                    n_steps,
                    n_draws,
                    seed
                )
        ...
        if "price" in st.session_state:
            st.success(
                f"Prix de l'option barrière {barrier_type} {barrier_exercise} : **{round(st.session_state['price'], 6)}**")
            ...
            if st.session_state["show_greeks"] or st.session_state["show_graphs"]:
                st.markdown(
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <h3>Greeks : Barrier Option</h3>
                        <h3>Graphs : Barrier Option</h3>
                    </div>
                    """,
